...
<CENTER>
<A NAME="TOP"></a>
<A HREF="http://www.jflex.de"><IMG SRC="logo.png" BORDER=0 HEIGHT=223 WIDTH=577></a>
</CENTER>
\end{rawhtml}
}
...
\smallskip\smallskip
Copyright \copyright 1998--2013 by \htmladdnormallink{Gerwin Klein}{http://www.doclsf.de}\\
\bigskip
...
\item
  ...
  1.2.1. See section \ref{performance} \htmlref{performance}{performance}
...
CharClassContent    ::= CharClass | (Character|Character'-'Character|MacroUsage|PredefinedClass)
...
UnicodePropertySpec ::= BinaryProperty | EnumeratedProperty (':' | '=') PropertyValue
...
\item
  ...
  \item
    Symmetric difference (\verb+~~+): the union minus the intersection,
    e.g. \verb+[\p{Letter}~~\p{ASCII}]+, equivalent to
    \verb+[[\p{Letter}||\p{ASCII}]--[\p{Letter}&&\p{ASCII}]]+: the
...
\item 
  ...
  \item
    ...
    \s  \p{Whitespace}
    \S  \P{Whitespace}
    \w  [\p{Alpha}\p{Digit}\p{Mark}\p{Connector Punctuation}\p{Join Control}]
    \W  [^\p{Alpha}\p{Digit}\p{Mark}\p{Connector Punctuation}\p{Join Control}]
    \end{verbatim}
...
Noncharacter\_Code\_Point, Default\_Ignorable\_Code\_Point, ANY, ASCII,
ASSIGNED.}
...
\subsection{RL1.2a Compatibility Properties}
...
\subsection{RL1.3 Subtraction and Intersection}
...
\subsection{RL1.4 Simple Word Boundaries}
...
boundary mechanism so that:}
  ...
  \begin{enumerate}
  \item
    \emph{The class of <word\_character> includes all the Alphabetic values
    from the Unicode character database, from UnicodeData.txt [UData],
...
\subsection{RL1.5 Simple Loose Matches}
...
\subsection{RL1.6 Line Boundaries}
...
\subsection{RL1.7 Supplementary Code Points}
...
the \texttt{\htmlref{\%cup}{CupMode}} directive a special meaning. However, an
...
This means the scanning function now changes its name from \texttt{yylex()}
...
\bibitem{unicode_rep}
  ...
  \htmladdnormallink{\texttt{http://www.unicode.org/unicode/reports/tr18}}
