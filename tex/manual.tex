...
of \texttt{"x"|"xy"} is also a prefix of \texttt{"yx"}. JFlex attempts
to report such cases at generation time, but it might be overeager: it
...
the text matched by the expression does not have a fixed size.  JFlex
attempts to report these cases as errors at generation time, but
the warnings are overeager. A large number of safe lookaheads are
reported as unsafe.
