...
\smallskip\smallskip
Copyright \copyright 1998--2004 by \htmladdnormallink{Gerwin Klein}{http://www.doclsf.de}\\
\bigskip
...
of \texttt{"x"|"xy"} is also a prefix of \texttt{"yx"}. In this case
JFlex will still correctly use \verb+r1 r2+ (\verb+r1+ followed by
\verb+r2+) to determine if the rule should be matched, but it might
return too many characters in \verb+yytext+ (it will return the
longest match of \verb+r1+ within \verb+r1 r2+).  JFlex attempts to
...
described in \cite{Aho} (leading to the deficiencies mentioned above).
...
example from the JLex website). The scanner was generated using JLex
...
expression matches a prefix of the trailing context and the length of
the text matched by the expression does not have a fixed size. In this
case JFlex will still correctly use \verb+r1 r2+ (\verb+r1+ followed
by a lookahead \verb+r2+) to determine if the rule should be matched,
but it might return too many characters in \verb+yytext+ (it will
return the longest match of \verb+r1+ within \verb+r1 r2+).  JFlex
...
\begin{itemize}
\item The check for unsafe lookahead expressions is overeager. 
  The lookahead algorithm itself works as advertised, but JFlex will report
  a large number of lookahead expressions as unsafe although they are safe.
