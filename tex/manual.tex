...
\usepackage{a4wide,verbatim,graphicx,microtype}
...
\newcommand{\ver}{1.7.0-SNAPSHOT}
\newcommand{\longver}{1.7.0-SNAPSHOT}
\newcommand{\nextver}{1.7}
\newcommand{\latestunicodever}{7.0}

\begin{document}

\latex{{\centerline{\hspace*{0mm}\includegraphics[width=0.6\textwidth]{jflex-black}}}}
...
\smallskip\smallskip
Copyright \copyright 1998--2015 by \htmladdnormallink{Gerwin Klein}{http://www.doclsf.de},
...
parser generators CUP, CUP2 and BYacc/J is explained in \htmlref{\emph{working
...
\phantom{}~~~~~~~~+\dd zero-reader\bs~~~~~~~~~~~~(example for Readers that return 0 characters)\\
...
{\ttfamily\htmlref{<YYINITIAL> "abstract"}{CodeRulesYYINITIAL}\verb+  {+ return symbol(sym.ABSTRACT); \verb+}+}
...
\item 
  ...
  \item
    ...
    Unicode properties (described \htmlref{below}{unipropsyntax}):
  ...
  \item
    ...
    Properties (described \htmlref{below}{unipropsyntax}):
...
\item \texttt{void yyreset(java.io.Reader reader)}\\
  closes the current input stream, and resets the scanner to read from a new
  Reader. All internal variables are reset, the old Reader \emph{cannot} be
  reused (content of the internal buffer is discarded and lost). The lexical
  state is set to \texttt{YY\_INITIAL}. The \verb+%{init+ code is \emph{not}
  included in \texttt{yyreset}, because it is assumed to run in the context
  of a constructor, not a normal method. If \verb+%{init+ does need to be
  repeated, consider constructing a new lexer object instead or additionally
  calling a custom function that performs any additional user-level state reset.
...
The link 
\htmladdnormallink{\texttt{http://docs.oracle.com/javase/1.5.0/docs/guide/intl/}}{http://docs.oracle.com/javase/1.5.0/docs/guide/intl/}
...
This section gives some \htmlref{tips}{PerformanceTips} on how to make
...
This section gives an incomplete overview of potential pitfalls and steps for
porting a lexical specification from the C/C++ tools \texttt{lex} and
\texttt{flex} \cite{flex} available on most Unix systems to JFlex.
...
The \texttt{user code} section usually contains C code that is used in actions
of the \texttt{rules} part of the specification. For JFlex, this code will have
to be translated to Java, and most of it will then go into the class code
\verb+%{..%}+ directive in the \texttt{options and dec\-la\-ra\-ti\-ons}
section.
...
same as in JFlex. Some attention is needed for escape sequences present in flex
(such as \verb+\a+) that are not supported in JFlex. These escape sequences
should be transformed into their octal or hexadecimal equivalent.

\subsubsection{Character Classes}
Flex offers the character classes directly supported by C, JFlex offers the
ones supported by Java. These classes will sometimes have to be listed manually.
...
\begin{itemize}
  \item in flex \verb+[][]+ is the character class containing \verb+]+ and
  \verb+[+, whereas in JFlex, the expression means ``empty expression''
  followed by ``empty expression''. To get \verb+]+ and \verb+[+ in JFlex, use
  for instance \verb+[\]\[]+.
  
  \item the classes \verb+[]+ and \verb+[^]+ are illegal in flex, but have
  meaning in JFlex.

  \item in flex \verb+["]+ is legal, in JFlex you need \verb+[\"]+.
\end{itemize}
...
therefore may not be consumed when \verb+^+ or \verb+$+ is present in a rule.
...
the \texttt{\htmlref{\%cup}{CupMode}} and \texttt{\htmlref{\%cup2}{CupMode}} directives
...
this interface automatically when the \texttt{\htmlref{\%cup}{CupMode}}
...
\htmlref{UTS\#18 Conformance}{unicoderegexconformance}.
...
See the file \htmladdnormallink{\texttt{COPYRIGHT}}{COPYRIGHT} for more
information.
...
\bibitem{unicode_rep}
  ...
  \htmladdnormallink{\texttt{http://www.unicode.org/reports/tr18/tr18-17.html}}
...
\bibitem{CUP}
  ...
  \htmladdnormallink{\texttt{http://www2.cs.tum.edu/projects/cup/}}
...
\bibitem{Jay}
  ...
  \htmladdnormallink{\texttt{http://www.cs.rit.edu/~ats/projects/lp/doc/jay/package-summary.html}}
...
\bibitem{UnicodeSet}
  {\trit Unicode Utilities: UnicodeSet}\\ 
   \htmladdnormallink{\texttt{http://unicode.org/cldr/utility/list-unicodeset.jsp}}
