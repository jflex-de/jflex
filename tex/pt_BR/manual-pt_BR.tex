...
\begin{verbatim}
C:\JFlex\                          
      +--bin\                   (scripts de inicializa\c{c}\~ao) 
      +--doc\                   (FAQ e manual)
      +--examples\ 
               +--binary\       (arquivos bin\'arios de leitura)
               ...
               +--java\         (especifica\c{c}\~ao do esc\^aner Java) 
               +--simple\       (examplo de esc\^aner)
               +--simple-maven\ (examplo com o Maven)
               +--standalone\   (um esc\^aner independente simples)
               +--standalone-maven\  (o mesmo de cima, com Mavem)
      +--lib\                   (classes precompiladas) 
      +--src\ 
          +--JFlex\             (c\'odigo-fonte do JFlex) 
          +--JFlex\gui          (c\'odigo-fonte das classes de interface gr\'afica do JFlex)
          +--java_cup\runtime\  (c\'odigo-fonte das classes de execu\c{c}\~ao do cup) 
\end{verbatim}
}
