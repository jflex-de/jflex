...
/**
 * Scans the common single-property Unicode.org data file format in DerivedAge.txt, populating
 * unicodeVersion.propertyValueIntervals for those property values representing Unicode versions
 * less than or equal to the Unicode version.
 */
public class DerivedAgeScanner extends EnumeratedPropertyFileScanner {
  ...
  /**
   * Returns true if the given property value, which is a major.minor Unicode version, is less than
   * or equal to the Unicode version, false otherwise.
   */
  @Override
  protected boolean accept(String propertyValue) {
    ...
    return unicodeVersion.majorVersion > major
        || (unicodeVersion.majorVersion == major && unicodeVersion.minorVersion >= minor);
  }

  /**
   * Specialized to make each version's code point ranges include all older versions' code point
   * ranges, and to handle overlapping code point ranges.
   */
  @Override
  public void addPropertyValueIntervals() {
    ...
    for (NamedRange interval : intervals) {
      ...
      NamedRangeSet versionedAgeRanges = ageRangesPerVersion.get(sortableVersion);
    ...
    NamedRangeSet highestVersionRanges = ageRangesPerVersion.get(ageRangesPerVersion.lastKey());
    NamedRangeSet unassignedRanges =
        new NamedRangeSet(new NamedRange(0, unicodeVersion.maximumCodePoint));
    ...
    for (NamedRange unassignedRange : unassignedRanges.getRanges()) {
      unicodeVersion.addInterval(
          propertyName, defaultPropertyValue, unassignedRange.start, unassignedRange.end);
    }
  }
