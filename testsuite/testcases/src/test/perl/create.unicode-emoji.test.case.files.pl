#!/usr/bin/perl
#
# create.unicode-emoji.test.case.files.pl
#
# This script is designed to take as input emoji-data.txt, and output
...
my $version = '';
my $emoji_data_filename = '';
my %properties = ();

GetOptions("version=s"=>\$version, "d=s"=>\$emoji_data_filename);

unless ($version && $emoji_data_filename && -f $emoji_data_filename
    && -r $emoji_data_filename)
{
    print STDERR "Usage: $0 -v <version> -d <unicode-Emoji-data-file>\n";
    exit(1);
}
...
open IN, "<$emoji_data_filename"
    || die "ERROR opening '$emoji_data_filename' for reading: $!";
while (<IN>)
{
    s/\s*\#.*//;
    next unless (/\S/);
...
for my $property (sort keys %properties)
{
    ...
    my $base_name
        = "EmojiData_${underscore_property}_${underscore_version}";
