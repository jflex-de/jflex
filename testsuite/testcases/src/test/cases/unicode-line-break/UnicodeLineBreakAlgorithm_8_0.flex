...
%unicode 8.0
%public
%class UnicodeLineBreakAlgorithm_8_0
%type String
...
%{
  ...
  public static void main(String argv[]) {
    if (argv.length == 0) {
      System.out.println("Usage : java UnicodeLineBreakAlgorithm_8_0 [ --encoding <name> ] <inputfile(s)>");
    }
    else {
      int firstFilePos = 0;
      ...
      UnicodeLineBreakAlgorithm_8_0 scanner = null;
      for (int i = firstFilePos; i < argv.length; i++) {
        try {
          ...
          while (null != (testLine = bufferedReader.readLine())) {
            ...
            if (null == scanner) {
              scanner = new UnicodeLineBreakAlgorithm_8_0(testReader);
            } else {
...
// (LB20 lookahead included)
//
\p{LB:SY} {CM}* \p{LB:HL} {CM}* / \p{LB:CB} { addMatch(); return nextSegment(); }
\p{LB:SY} {CM}* \p{LB:HL} {CM}* / [^] { addMatch(); }
\p{LB:SY} {CM}* \p{LB:HL} {CM}* { addMatch(); }

// LB22  Do not break between two ellipses, or between letters or numbers and ellipsis.
//
// (AL | HL) × IN  ;  EX × IN  ;  ID × IN  ;  IN × IN  ;  NU × IN
//
// Examples: ‘9...’, ‘a...’, ‘H...’
//
(({AL} | [\p{LB:HL}\p{LB:EX}\p{LB:ID}\p{LB:IN}\p{LB:NU}]) {CM}* | {CM}+) / \p{LB:IN} { addMatch(); }
