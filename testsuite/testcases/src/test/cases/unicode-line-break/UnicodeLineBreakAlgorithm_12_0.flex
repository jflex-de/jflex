...
%unicode 12.0
%public
%class UnicodeLineBreakAlgorithm_12_0
%type String
...
%{
  ...
  public static void main(String[] argv) {
    if (argv.length == 0) {
      System.out.println("Usage : java UnicodeLineBreakAlgorithm_12_0 [ --encoding <name> ] <inputfile(s)>");
    }
    else {
      int firstFilePos = 0;
      ...
      UnicodeLineBreakAlgorithm_12_0 scanner = null;
      for (int i = firstFilePos; i < argv.length; i++) {
        try {
          ...
          while (null != (testLine = bufferedReader.readLine())) {
            ...
            if (null == scanner) {
              scanner = new UnicodeLineBreakAlgorithm_12_0(testReader);
            } else {
