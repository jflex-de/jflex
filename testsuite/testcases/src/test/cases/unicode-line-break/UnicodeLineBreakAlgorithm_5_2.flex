...
%unicode 5.2
%public
%class UnicodeLineBreakAlgorithm_5_2
%type String
...
%{
  ...
  public static void main(String argv[]) {
    if (argv.length == 0) {
      System.out.println("Usage : java UnicodeLineBreakAlgorithm_5_2 [ --encoding <name> ] <inputfile(s)>");
    }
    else {
      int firstFilePos = 0;
      ...
      UnicodeLineBreakAlgorithm_5_2 scanner = null;
      for (int i = firstFilePos; i < argv.length; i++) {
        try {
          ...
          while (null != (testLine = bufferedReader.readLine())) {
            ...
            if (null == scanner) {
              scanner = new UnicodeLineBreakAlgorithm_5_2(testReader);
            } else {
...
// in Section 5.8, Newline Guidelines, of [Unicode5.2]. These three rules are
...
// ZW ÷
//
// Note: The break opportunities produced by LB8 differ in certain cases from
// those produced by the pair table included in Section 7, Pair Table-Based 
// Implementation. The differences occur with sequences like ZW SP CL. 
// The inconsistencies will be addressed in the next revision of this document.
//
...
// (CL | CP) SP* × NS
//
[\p{LB:CL}\p{LB:CP}] {CM}* \p{LB:SP}* / \p{LB:NS} { addMatch(); }
...
//
\p{LB:SP} / {CM}+ [\p{LB:BK}\p{LB:CR}\p{LB:LF}\p{LB:NL}\p{LB:SP}\p{LB:ZW}\p{LB:WJ}\p{LB:CL}\p{LB:CP}\p{LB:EX}\p{LB:IS}\p{LB:SY}\p{LB:QU}\p{LB:BA}\p{LB:HY}\p{LB:NS}] { addMatch();  return nextSegment(); }
...
//
({AL}|\p{LB:NU}) {CM}* / \p{LB:OP} { addMatch(); }
\p{LB:CP} {CM}* / ({AL}|\p{LB:NU}) { addMatch(); }
...
//
[^\p{LB:BK}\p{LB:CR}\p{LB:LF}\p{LB:NL}\p{LB:ZW}] {CM}* / [\p{LB:BK}\p{LB:CR}\p{LB:LF}\p{LB:NL}\p{LB:SP}\p{LB:ZW}\p{LB:WJ}\p{LB:CL}\p{LB:CP}\p{LB:EX}\p{LB:IS}\p{LB:SY}\p{LB:QU}\p{LB:BA}\p{LB:HY}\p{LB:NS}] { addMatch(); }
