...
%unicode 6.3
%public
%class UnicodeLineBreakAlgorithm_6_3
%type String
...
%{
  ...
  public static void main(String argv[]) {
    if (argv.length == 0) {
      System.out.println("Usage : java UnicodeLineBreakAlgorithm_6_3 [ --encoding <name> ] <inputfile(s)>");
    }
    else {
      int firstFilePos = 0;
      ...
      UnicodeLineBreakAlgorithm_6_3 scanner = null;
      for (int i = firstFilePos; i < argv.length; i++) {
        try {
          ...
          while (null != (testLine = bufferedReader.readLine())) {
            ...
            if (null == scanner) {
              scanner = new UnicodeLineBreakAlgorithm_6_3(testReader);
            } else {
