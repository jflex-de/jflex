...
public final class LR1Item implements Item {
  ...
  /**
   * Creates a new {@link LR1Item}, using the given {@link Production}, position within this
   * production and set of possible lookahead terminals.
   */
  ...
  /**
   * Creates a new {@link LR1Item}, using the given {@link LR0Item} and set of possible lookahead
   * terminals.
   */
  ...
  /**
   * Returns a new {@link LR1Item}, having the same LR(0) kernel as this one and containing the
   * union of the lookahead symbols of this item and the given ones. If no new lookaheads are given,
   * the original LR1Item (<code>this</code>) is returned.
   */
  public LR1Item createUnion(TerminalSet lookaheads) {
    if (Objects.equals(this.lookaheads, lookaheads)) {
      return this;
    } else {
  ...
  /**
   * Gets the FIRST set of βa (where this item has the following form: "A → α.Xβ with lookahead a"
   * with X being the next symbol), using the given precomputed nullable set and FIRST sets for all
   * symbols.
   */
  ...
  /**
   * Gets the FIRST set of βa (where this item has the following form: "A → α.Xβ with lookahead a"
   * with X being the next symbol). See </code>getNextLookaheads</code>.
   */
  TerminalSet computeNextLookaheadSymbols(FirstSets firstSets, NullableSet nullableSet) {
    ...
    for (int i = kernel.position + 1; i < kernel.production.getRHS().size(); i++) {
      ...
      if (!(symbol == Epsilon
          || nullableSet.contains(
              symbol))) // TODO: why "symbol == Epsilon"? redundant to nullableSet
      return ret;
    }
  ...
  public LR1Item merge(LR1Item item) {
    if (!Objects.equals(kernel, item.kernel))
  ...
  public boolean equals(Object obj) {
    if (obj instanceof LR1Item) {
      ...
      if (!Objects.equals(kernel, l.kernel)) return false;
      if (!Objects.equals(lookaheads, l.lookaheads)) return false;
      return true;
    }
    return false;
  }
  ...
  public boolean equalsLR0(LR1Item item) {
    return Objects.equals(kernel, item.kernel);
  }
