...
/**
 * Factory for a LALR(1) automaton, that improves on {@link LALR1CPAutomatonFactory} by computing
 * the context propagation with a StronglyConnectedComponents approach instead of slow fixpoint
 * iteration.
 *
      ...
      do {
        ...
        while (!(dfsStack.isEmpty())) {
          ...
          if (found == null) { // not found a child
            ...
            if (!dfsStack.isEmpty()) {
              lowlink.put(
                  dfsStack.peek(), Math.min(lowlink.get(dfsStack.peek()), lowlink.get(item)));
            }

            if (Objects.equals(lowlink.get(item), dfsIndices.get(item))) {
