...
/**
 * A production consists of a left hand side {@link NonTerminal} and a list of right hand side
 * {@link Symbol}s. There may be on single {@link Action} which is performed when the production is
 * reduced.
 *
 * <p>If there are productions with more than one right hand sides (in parser books separated by
 * "|"), each of these sides must be represented by its own {@link Production} instance.
 *
 * <p>A production should have an index, to support debugging and parsing table dumps.
 *
 * <p>TODO: TIDY constructors
 *
...
public final class Production implements Serializable {
  ...
  public Production(
      int id, NonTerminal lhs, List<Symbol> rhs, Action reduceAction, Terminal precTerminal) {
    // check rhs
    if (rhs.size() == 0) {
      ...
      for (Symbol symbol : rhs) {
        if (symbol == SpecialTerminals.Epsilon) {
          throw new IllegalArgumentException(
              "Epsilon is only allowed as the single symbol of a RHS!");
        }
      }
  ...
  /**
   * Convenience constructor: Just give an ID, a left hand side {@link NonTerminal} and a list of
   * right hand side {@link Symbol}s. The {@link Production} contains no {@link Action}s.
   */
  ...
  @SuppressWarnings("unchecked")
  private void deserializeReduceAction(ObjectInputStream stream)
    ...
    try {
      ...
      Constructor<CUP2Specification> c_enclosing =
          (Constructor<CUP2Specification>) clazz.getEnclosingConstructor(); // gets
      // constructor
      // for
      // specification
      // System.out.println("constructor enclosing : "+c_enclosing);
      Class<CUP2Specification> clazz_enclosing =
          (Class<CUP2Specification>) clazz.getEnclosingClass();
      if (c_enclosing == null) {
      ...
      Constructor<?>[] c_declared =
          clazz.getDeclaredConstructors(); // 1 constructor - NOT public, NOT default (expects
      // reference to outer object)
      for (Constructor<?> c : c_declared) {
        Class<?>[] cParms = c.getParameterTypes();
        if (cParms.length == 1) {
          if (Objects.equals(cParms[0], clazz_enclosing)) {
            // creating a new instance of Specification is ugly, if Specification should be
            // serialized:
  ...
  /** Returns the number of right hand side symbols, without epsilons. */
  ...
  public Action getAction(int position) {
    if (position < this.rhs.size()) return null;
    else return reduceAction;
  }
  ...
  /**
   * Returns the Terminal responsible for the precedence rating of this production, or null if there
   * is none.
   */
  ...
  public boolean equals(Object obj) {
    if (obj instanceof Production) {
      ...
      if (!Objects.equals(rhs, p.rhs)) return false;
      return true;
    }
    return false;
  }
  ...
  public String toString() {
    ...
    for (Symbol s : rhs) rhsString.append(s + " ");
  ...
  public String toString(int dotPosition) {
    ...
    for (Symbol s : rhsSymbols) {
      if (dotPosition == rhsSymbols.getIndex()) rhsString.append(". ");
      rhsString.append(s + " ");
    }
    if (dotPosition == rhs.size()) rhsString.append(". ");
    return "Production " + id + ": " + lhs + " → " + rhsString;
  }
}
