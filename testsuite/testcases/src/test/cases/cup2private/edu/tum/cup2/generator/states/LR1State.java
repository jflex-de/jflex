 ...
 *
 * <p>Methods related to the nucleus were removed (since the author of this class doesn't know
 * anything about them).
 *
 * @author Andreas Wenger
 */
public class LR1State extends LRState<LR1Item> {
  ...
  public LR1State closure(GrammarInfo grammarInfo) {
    if (closureCache == null) closureCache = new LR1State(closureItems(grammarInfo));
    return closureCache;
  }

  /**
   * Returns the closure of the {@link LR1Item}s of this state as set of {@link LR1Item}s. For a
   * description of this algorithm, see Appel's book, page 63.
   *
   * <p>TODO: optimization possible by using persistent data structures?
   */
      ...
      if (nextSymbol instanceof NonTerminal) // nextSymbol is "X"
      {
            ...
            grammarInfo.getProductionsFrom((NonTerminal) nextSymbol)) // p is each "X → γ"
        {
          ...
          if (sameKernelItemLookaheads != null) {
            ...
            if (!Objects.equals(newLookaheads, sameKernelItemLookaheads)) {
   ...
   * as a new state (only the kernel, without closure). For a description of this algorithm, see
   * Appel's book, page 62.
   */
  @Override
  ...
  /**
   * Gets the number of items (when each item has only one lookahead symbol, e.g. an item with 3
   * lookahead symbols counts as 3 items). Useful for debugging and testing.
   */
  ...
  /**
   * Returns true, if this state contains an item with the given production, position and lookahead
   * symbol. Useful for debugging and testing.
   */
  boolean containsSimpleItem(Production production, int position, Terminal lookahead) {
    for (LR1Item item : items) {
      if (item.getProduction() == production
          ...
          && item.getLookaheads().contains(lookahead)) return true;
    }
    return false;
  }

  /**
   * Gets the item of this state which has the given LR(0) kernel, or <code>null</code>, if there is
   * no such item.
   */
  ...
  /**
   * Merges this state with the given one and returns the result. Only works, if both states have
   * equal LR(0) kernels.
   */
  ...
  public boolean equals(Object obj) {
    if (obj instanceof LR1State) {
      ...
      for (LR1Item item : items) {
        if (!s.items.contains(item)) return false;
      }
      return true;
    }
    return false;
  ...
  /**
   * Returns true, when the LR(0) kernel of the state is equal to the LR(0) kernel of the given
   * state.
   */
  public boolean equalsLR0(LR1State state) {
    // first check for same hash code for performance reasons
    return kernel.hashCode() == state.kernel.hashCode() && Objects.equals(kernel, state.kernel);
  }
}
