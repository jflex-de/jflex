...
%unicode 6.1
%public
%class UnicodeGraphemeBreakRules_6_1
%type String
...
%{
  ...
  public static void main(String argv[]) {
    if (argv.length == 0) {
      System.out.println("Usage : java UnicodeGraphemeBreakRules_6_1 [ --encoding <name> ] <inputfile(s)>");
    }
    else {
      int firstFilePos = 0;
      ...
      UnicodeGraphemeBreakRules_6_1 scanner = null;
      for (int i = firstFilePos; i < argv.length; i++) {
        try {
          ...
          while (null != (testLine = bufferedReader.readLine())) {
            ...
            if (null == scanner) {
              scanner = new UnicodeGraphemeBreakRules_6_1(testReader);
            } else {
...
// GB9b. 	Prepend 	× 	 
//
// Unicode 6.1 has no GCB:Prepend chars, so this is not a valid property under JFLex
//
