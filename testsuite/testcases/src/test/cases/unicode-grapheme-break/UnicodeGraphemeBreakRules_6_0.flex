...
%unicode 6.0
%public
%class UnicodeGraphemeBreakRules_6_0
%type String
...
%{
  ...
  public static void main(String argv[]) {
    if (argv.length == 0) {
      System.out.println("Usage : java UnicodeGraphemeBreakRules_6_0 [ --encoding <name> ] <inputfile(s)>");
    }
    else {
      int firstFilePos = 0;
      ...
      UnicodeGraphemeBreakRules_6_0 scanner = null;
      for (int i = firstFilePos; i < argv.length; i++) {
        try {
          ...
          while (null != (testLine = bufferedReader.readLine())) {
            ...
            if (null == scanner) {
              scanner = new UnicodeGraphemeBreakRules_6_0(testReader);
            } else {
