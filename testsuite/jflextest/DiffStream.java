...
class DiffStream {
   ...
   * 
   * @param diffLines   a Vector of Integer. The line numbers where
   *                    differences are expected (and to be ignored) 
   * @param input       the input stream
   * @param expected    the control stream
   */
	public String diff(Vector diffLines, Reader input, Reader expected) {
    if (diffLines == null) diffLines = new Vector();
		linesProcessed = 1;
