...
/**
 * CheckedGrammar also has additional information (reachability and productivity) for symbols,
 *
 * <p>CheckedGrammar is a decorator for Grammars
 *
...
public class CheckedGrammar implements IGrammar {
  ...
  private Hashtable<Symbol, SymbolInfo> symbolInfos = new Hashtable<Symbol, SymbolInfo>();
  ...
  /**
   * Creates a new {@link Grammar}, using the given list of {@link Production}s. The first
   * production is the start production of the grammar.
   */
  public CheckedGrammar(
   ...
   */
  public CheckedGrammar(
      Terminal[] terminals, NonTerminal[] nonTerminals, Production[] productions) {
    this(
        ArrayTools.toLinkedListT(terminals),
        ArrayTools.toLinkedListT(nonTerminals),
        ArrayTools.toLinkedListT(productions));
  }

  /**
   * SymbolInfo stores additional information to symbols in the grammar e.g: is a NonTerminal
   * reachable?
   *
   * @author Michael Hausmann
   */
  private class SymbolInfo {
    ...
    public boolean equals(Object obj) {
      ...
      return (Objects.equals(this.symbol, otherSymbolInfo.symbol));
    }
  } /* end of class */
  ...
  /**
   * set NonTerminal lhs reachable and also set dependent symbols reachable recursively
   *
   * @param lhs
   */
  ...
  /**
   * getReachableNonTerminals returns a LinkedList of reachable NonTerminals
   *
  ...
  public LinkedList<NonTerminal> getReachableNonTerminals() {
    ...
    for (SymbolInfo i : c) {
      if ((i.symbol instanceof NonTerminal) && i.bReachable) {
  ...
  /**
   * getNotReachableNonTerminals returns a LinkedList of n o t reachable NonTerminals
   *
   * @return list of reachable NonTerminals
   */
  ...
  public boolean hasNotProductiveNonTerminals() {
    ...
    Collection<SymbolInfo> c = this.symbolInfos.values();
    for (SymbolInfo i : c) {
      if ((i.symbol instanceof NonTerminal) && !i.bProductive) {
        return true;
      }
    }
    return false;
   ...
   *
   * @return true if the grammar is reduced; i.e: has no unreachable and no unproductive
   *     NonTerminals
   */
  ...
  /**
   * getProductiveNonTerminals returns a LinkedList of productive NonTerminals
   *
  ...
  /**
   * getNotProductiveNonTerminals returns a LinkedList of n o t productive NonTerminals
   *
   * @return list of productive NonTerminals
   */
  ...
  /**
   * getProductiveAndReachableNonTerminals returns a LinkedList of productive and reachable
   * NonTerminals
   *
  ...
  public LinkedList<NonTerminal> getProductiveAndReachableNonTerminals() {
    ...
    Collection<SymbolInfo> c = this.symbolInfos.values();
    for (SymbolInfo i : c) {
      if ((i.symbol instanceof NonTerminal) && i.bProductive && i.bReachable) {
        ret.add((NonTerminal) i.symbol);
      }
    }
    return ret;
  ...
  /**
   * checkProductivity checks the productivity of each NonTerminal using the algorithm from the
   * compiler construction lecture (page 92). (fixed point iteration)
   */
  protected void checkProductivity() {
    ...
    // NOTE: if NT occurs multiple times in one production, then the list contains the productions
    // multiple times
    ...
    for (Entry<Production, Integer> r : count.entrySet()) {
      if (Objects.equals(r.getValue(), 0)) {
        if (DEBUG) System.out.println("working set entry: " + r.getKey());
        W.add(r.getKey());
      }
    }
    ...
    while (W.size() > 0) {
      Production Ai =
          W.removeFirst(); // extract one rule of a productive NonTerminal from the Workset
      NonTerminal A = Ai.getLHS();
      if (!(result.contains(A))) {
        result.add(A);
        if (DEBUG) System.out.println(A + " productive");
        for (Production r :
            rhs.get(A)) // get productions that contain the productive NT A in their rhs
        {
          Integer i = count.get(r);
          assert (i != null); // i != null  must be valid
          i--; // for every occurrence of A decrease the counter
          // NOTE: if production r contains A more than once, this loop body is executed also more
          // than once

          count.put(r, i);
          if (DEBUG) System.out.println(" -> " + r + " (" + i + ")");
          if (i == 0) W.add(r);
        }
      }
    }
  ...
  public IGrammar getReducedGrammar() {
    ...
    for (Production p : allProductions) {
      ...
      if (!requiredNonTerminals.contains(lhs)) bKeepProduction = false;
      else {
        for (Symbol s : rhs) {
          if (s instanceof NonTerminal) {
