...
/** CUP v0.10k generated parser.
  * @version Thu Jun 19 22:39:53 CEST 2003
  */
...
class CUP$LexParse$actions {
  ...
  private void makeCaselessClasses(String s) {
    for (int i = 0; i < s.length(); i++)
      makeCaselessClass(s.charAt(i));    
  }

  private void makeCaselessClass(char c) {
    char upper = Character.toUpperCase(c);
    char lower = Character.toLowerCase(c);
    char title = Character.toTitleCase(c);

    Vector ccl = new Vector();
    ccl.addElement( new Interval(upper,upper) );
    ccl.addElement( new Interval(lower,lower) );
    ccl.addElement( new Interval(title,title) );
